...
public interface Validation<E, T> extends Serializable {
    ...
    static <E, T> Validation<E, T> invalid(E error) {
        ...
        return new Invalid<>(Stream.of(error).collect(Collectors.toList()));
    }
    ...
    static <E, T> Validation<E, T> invalid(E... errors) {
        ...
        return invalidAll(Stream.of(errors).collect(Collectors.toList()));
    }
    ...
    static <E, T> Validation<E, T> invalidAll(Iterable<? extends E> errors) {
        Objects.requireNonNull(errors, "errors is null");
        final List<E> errorList = (errors instanceof List) ? (List<E>) errors : createList(errors);
        return new Invalid<E, T>(errorList);
    }

    static <T> List<T> createList(Iterable<? extends T> iterable) {
     ...
     */
    default T getOrElseGet(Function<? super List<? super E>, ? extends T> other) {
        Objects.requireNonNull(other, "other is null");
        if (isValid()) {
            return get();
        } else {
     ...
     */
    default <U> U fold(Function<? super List<? super E>, ? extends U> fInvalid, Function<? super T, ? extends U> fValid) {
     ...
     */
    default Validation<T, List<E>> swap() {
        if (isValid()) {
            return invalid(get());
        } else {
    ...
    default <U> Validation<U, T> mapErrors(Function<? super E, ? extends U> f) {
        ...
        if (isValid()) {
            ...
            return invalidAll(getErrors().stream().map(f).collect(Collectors.toList()));
        }
    }
    ...
    default <E2, T2> Validation<E2, T2> bimap(
        ...
        if (isValid()) {
            ...
            return invalidAll(getErrors().stream().map(errorMapper).collect(Collectors.toList()));
        }
    }
    ...
    default Validation<E, T> filterErrors(Predicate<? super E> predicate) {
        ...
        if (isValid()) {
            ...
            return invalidAll(getErrors().stream().filter(predicate).collect(Collectors.toList()));
        }
    }
    ...
    default <U> Validation<U, T> flatMapErrors(Function<? super E, ? extends Iterable<? extends U>> f) {
        ...
        if (isValid()) {
            ...
            List<U> errors = new ArrayList<>();
            getErrors().stream().map(f).map(v -> createList(v)).forEach(list -> errors.addAll(list));
            return invalidAll(errors);
        }
    }
    ...
    final class Valid<E, T> implements Validation<E, T>, Serializable {
        ...
        @Override
        public List<E> getErrors() throws RuntimeException {
